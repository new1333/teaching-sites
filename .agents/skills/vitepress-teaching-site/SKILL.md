...
- **权威文档清单**（主题涉及客观事实断言时才有）：该主题的权威资料列表（硬件文档、协议规范、语言标准等），备课期定。写作期凡断言「真机/规范如此」，依据只能是这份清单——**测试通过与实现一致不构成事实正确的证据**（测试与实现同错时会互相背书）。纯主观工程实践类主题可缺省此条。
...
1. **spawn 章写作智能体**：整章一次做完——快照 → 写本章测试（先红，自设原理断言、不从任何官方/上游测试改编；`tests/` append-only，绝不动旧章测试）→ 演进伴生实验场（转绿）→ 双硬门槛（失败回灌最多 3 轮，仍败回滚快照、降级为占位章，**不中断全书**）→ 写正文（≥1200 字、上限不设；骨架=痛点开章→原理→渐进实验→验证（双门槛两侧可见；里程碑可感知时给「亲手开机」指引）→小结；new_concepts 按「陌生概念怎么教」四步处理；承诺面=交付面、承重知识给自包含载体、事实断言对照权威文档；专业名词首现说人话；**零外部源码引用**；标注 `src/…` 的代码必须与实验场终态一致）→ lint 脚本机械自检（带 `--pain` 传该章痛点现象词）。门槛与写作细则见 `references/companion-and-gates.md` 与 `references/chapter-writing.md`。返回：产物路径 + rolling_summary 草稿 + 降级报告（如有）。写作智能体是全新会话，每次 spawn 先读 chapter-writing.md——原「每会话首读」纪律自动成立。
2. **spawn 评审智能体**：新鲜眼按三轴（概念教学最重 / 实现完整性 / 结构契约）评审本章落盘产物，文验收条目逐条回查回报兑现状态，产出阻断/建议清单（分级与格式见 `references/subagents.md`）。占位章跳过。
...
全部章完成后，spawn 全书评审智能体（同样新鲜眼，范围=全部章 + outline + bible + 整个 companion）：概念链跨章核对、术语与 glossary 一致、读者成长线兑现、文风漂移、**正文↔代码引用全书全量比对**（重构回写是否漏网）、**数字与门槛命令实测一致**、**acceptance 全书回查**、**附录对账与互链无死结**；修订路由同阶段 3 第 3 步。并行模式在此前先由主智能体做缝合审计——蓝图计划摘要 vs 实际代码、API 演进表 vs 实际导出、概念首秀表 vs 实际正文，机械比对、现实是事实源（细则见 `references/subagents.md`）。
...
2. 终检（完整清单见 `references/vitepress-assembly.md`，此处列口径）：章文件数与 frontmatter title 与大纲一致、术语表全书出现、companion 全量门槛通过、**零仓库痕迹**（repo 输入时 grep 抽查 docs/ 与 companion/——无 clone 仓库的路径、无「与真源码对照」、无改编自官方测试的说法）、**数字事实核对**（脚本执行：正文的用例数/累计数/行数与 vitest 输出、`wc -l` 比对，不一致改文）、**正文↔代码一致性**（src 标注全量比对终态）、**acceptance 逐条回查**、**附录对账**（速查表与实现一致、差异清单齐全、互链无死结）、**companion 资产清白**（无超纲产物、无版权输入物）、**零输入体验**（形态允许时终章产物自带课程自产内置输入，访客不自备文件就能看到成果；不允许时 README 写明）、**可感知成果落点**（里程碑可感知的章有「亲手开机」指引；渲染/音频/可视化类正文嵌真实产出的资产，聚合站 base 兼容）。
