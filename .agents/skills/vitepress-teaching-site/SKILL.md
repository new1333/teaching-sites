...
```
输入（repo URL / 主题句）
 → 阶段 0 备课：特性清单（8-15 个）+ 能力依赖图      ← repo 读仓库（仅作者侧备课资料）/ topic 直接拆解，两路汇流
...
- **仓库输入**：clone 后有节制地读（入口识别 → 关键文件 ≤18 个 → 特性抽取）。读法纪律见 `references/repo-ingestion.md`——备课阶段的克制决定了后面 10 章还有没有上下文可用。**仓库只到此为止**：clone 与备课笔记是作者侧资料，正文、测试、实验场命名一律零仓库痕迹。
...
产出 CourseOutline JSON：8-12 章、2-4 分部、principle:build ≈ 3:7、每章带 pain_point / milestone / depends_on / acceptance，外加原理实验场的形态与规模。Schema、硬性规则与金样例见 `references/outline-schema.md`。验收测试不在此阶段预写——阶段 3 每章现写（先红后绿），渐进语义由红→绿机械背书，无需人工审计 import。

向用户呈现：分部章节表（`# | 章 | 类型 | 痛点 | 里程碑`）+ 终点里程碑 + 实验场形态与规模（语言/形态/预计行数，如「Go 的最小调度器，~300 行」）。反馈循环：整纲反馈 → 带反馈重生成整纲（反馈必须落实）；单章不满 → 只改该章条目。收敛不出可机械验证的原理实验形态时，按「何时不适用」与用户前置澄清，不要硬生成。确认后进入阶段 3。
...
3. **写正文**：≥1200 字、上限不设，骨架=痛点开章→原理→渐进实验（引用伴生实验场的真实代码）→验证→小结；**零外部源码引用**——示例代码只出自实验场或自包含用法示例，不写「与真源码对照」段落。写作纪律与笔感基准见 `references/chapter-writing.md`——**每会话首次动笔前读一次**（长时间中断或 lint 连续报警再重读，不必每章读）。
...
2. 终检：章文件数与大纲一致、每章 frontmatter title 与大纲一致、术语表条目全书出现过、companion 全量门槛通过（按形态执行）、**零仓库痕迹**（repo 输入时 grep 抽查 docs/ 与 companion/——无 clone 仓库的路径、无「与真源码对照」、无改编自官方测试的说法）。
