...
2. 本章 spec：{course_dir}/.course/outline.json 里 slug 为 {slug} 的章（hook / milestone / new_concepts / misconceptions /
   structure / acceptance 全在里面）与顶层 profile（archetype / verification / source_policy / code_density / obligations）。
...
1. **概念教学**（最重，跨形态不变量）：spec 的 new_concepts 是否按四步（成因→载体→演算→锚点）真教了；**承重概念分级**——本章结论/里程碑依赖的概念（无论大纲是否声明）是否逃过了一句话兜底、成因是否过了反事实检验（「如果不这样会怎样」答不出却写了「因为」即阻断，伪成因禁绝）；其余专业名词首现是否当场一句人话——**费曼检验**：首现段念给外行听、复述不出大意即阻断；读者模型陌生清单里的前置概念是否被默认已会。**误区证伪**：spec 声明的 misconceptions 逐条核对——误解被公允复述且被验证物/演算真实证伪，一笔带过不算。**承诺面对齐**：本章以标题、总表、开篇承诺列出的每一项在正文有着落，静默缩水即阻断；**待清承诺**：主智能体注入的 promises.json 清账项逐条核对兑现状态。**自包含载体**：承重知识点有 REPL 片段或数值演算兜底。**自查军规**：自查题不得正文原句可抄、题干不含答案、每问有答案锚点/<details>。**文验收回查**：spec acceptance 里的正文教学类条目逐条核对，回报兑现/部分/未兑现——未兑现即阻断。
...
3. **结构契约**：开章钩子具体到现象（kind 不限）；收束槽回应开篇钩子（抛而不收记建议项）；一章一特性；出处纪律（zero-trace：零外部源码引用；walkthrough：引用与锁定 ref 逐字一致）；骨架五槽位齐全（该章声明了 structure 时按声明的结构验收）、验证槽含读者动作与「先猜后跑」。
