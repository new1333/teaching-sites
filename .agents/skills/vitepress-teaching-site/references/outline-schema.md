...
Chapter = {
  ...
  acceptance: string[]  // 少量可判定的验收项（含「实验场门槛通过」）
}
```
...
1. **一章一特性，章数由特性数决定**：备课拆出几个值得单独成章的核心特性就几章——不设上下限，不为凑数拆章，也不为省事并章；分部按学习的自然阶段划分，几个由内容决定。内容多就多几章，这是特性，不是问题。
...
7. acceptance 少而可判定（通常 3-5 条就够），至少含「实验场门槛通过」与「零外部源码引用」。
