# 伴生实验场与双硬门槛

执行者：**章写作智能体**（串行模式整章执行；并行模式拆为代码线=本循环全部、正文线=写正文+lint——模板与文件所有权见 `subagents.md`）。

## 伴生实验场是什么

`companion/` 是**原理实验场**——读者课程结束时拥有的最小实验工程（如验证状态管理原理的最小 store 容器 ≈300 行）。它的存在只为回答一个问题：**这条原理真的成立吗？**围绕备课阶段拆出的特性清单逐章搭建，规模几百行；API 命名服务于教学清晰，不与任何真实库刻意对齐，也不追求功能完整——够验证原理即可。它是全书的共享状态：每章演进一次，正文的实验段落引用它的真实代码。

为什么先代码后文：长篇课程的一致性无法靠模型记忆或长上下文保证——门槛命令和测试是外部硬信号，代码连续性由它们背书。所以每个 build 章都是两次生成：**先演进实验场并过门槛，再基于真实代码变更写正文**。

## 形态按主题选（不变量：每章有机械可验证的增量）

| 形态 | 适用 | 双硬门槛示例 |
|---|---|---|
| library（默认） | 库 / 框架内核 | `tsc --noEmit` + `vitest run`；Go 等价物 `go vet` + `go test ./...` |
| cli-golden | CLI、解析器、构建工具 | 构建 + golden 输出比对（固定输入 → 比对输出文件） |
| config-validate | Nginx / Terraform / k8s 配置类 | `nginx -t` / `terraform validate` / `kubectl apply --dry-run=client` |
| dom-test | 浏览器组件、可视化 | 构建 + DOM 断言（vitest + jsdom / testing-library） |

不变量只有一条：**门槛是外部的、机械的，过了才写正文**。语言跟随主题的自然语言。形态与规模写进大纲的 `companion` 字段、随章节表一并确认；若任何形态都收敛不出，按 SKILL.md「何时不适用」与用户前置澄清，不要硬生成。

## 脚手架（阶段 2 确认后、第一章之前建好）

以默认 library 形态（TS + vitest）为例；其他形态用等价脚手架（go.mod、Makefile 等），门槛命令按上表。

`companion/package.json`：

```json
{
  "name": "companion",
  "private": true,
  "type": "module",
  "scripts": { "typecheck": "tsc --noEmit", "test": "vitest run" },
  "devDependencies": { "typescript": "^5.6.0", "vitest": "^2.1.0" }
}
```

`companion/tsconfig.json`：`strict: true`、`target/module: ESNext`、`moduleResolution: bundler`、include `src` 与 `tests`。建好后跑一次 `npm install`（后续章不再装）。

## 每章门槛循环（build 章）

1. **快照**：复制 `companion/src` 到 `.course/snapshots/pre-ch-{N}/`（失败回滚点）。
2. **写本章测试（红）**：现写 `companion/tests/{slug}.test.ts`，断言该章 milestone 的**行为**；先跑一次，必须失败——这就是渐进语义的机械证明，无需人工审计 import。`tests/` 是 append-only：只新增本章测试，绝不动旧章测试；旧章测试持续全绿，就是公共 API 向后兼容的哨兵。
3. **演进代码**，纪律：
   - 文件级变更、增量演进——不重写与本章无关的文件；
   - 不改 `tests/` 里旧章的测试文件（测试由管线管理）；
   - 遵守圣经的代码约定，与既有文件风格/命名一致；
   - 备课文件（repo 输入时，从 clone 里读该章 relevant_files，≤4 个，各截 ~12k 字符）仅在拿不准某条原理的真实行为时查阅——**不照抄其代码、不改编其测试**；成稿零仓库痕迹。
4. **双硬门槛**（cwd = `companion/`，按形态执行对应命令）：先确认本章新测试已转绿，再全量跑一遍——全部测试必须通过。
5. 失败：把报错尾部（门槛命令全量输出 + 测试最后 ~50 行）作为下一轮修复的输入，回到步骤 3。**最多 3 轮**。
6. 3 轮仍败：**回滚快照** → 本章降级为占位章（正文写为 `::: warning` 块，含失败原因摘要与重试指引）→ 滚动摘要注明「实验场保持上一章形态」→ **继续下一章，不中断全书**。
7. **写完正文后过引用一致性闸门**（lint 的 snippet-missing 检查 + 评审智能体的一致性核对）：正文标注 `src/…` 的代码块必须与伴生仓**当前形态**一致（教学注释除外）。后续章重构波及「已被前章正文引用的函数」时，必须回写受影响的前章正文再继续——串行模式由主智能体在下一章开工前回扫，并行模式并入缝合审计。实验场演进后正文还挂着旧形态的代码，是最伤信任的缺陷之一。

## 重构回写义务

伴生仓是全书共享状态，第 N 章的重构可能改掉第 M（M＜N）章正文引用过的函数。规则只有一条：**正文引用的代码必须始终等于终态**。触发即回写——改正文里的代码块为当前形态（必要时顺带补两句新词汇的解释），修完重跑该章 lint。快照目录（`.course/snapshots/`）是内部状态，读者不可达——「本章当时的形态」不能作为正文引用的出处；确需展示历史形态时按「拼版·教学示意」标注并注明终态去向（chapter-writing 硬要求 12）。

## 测试写法要点（写「红」测试时的自检）

- 每个 build 章一个 `tests/{slug}.test.ts`；只依赖实验场自身，不依赖网络、不 sleep 等待；
- **测试自设**：断言原理行为（「scope.stop 之后 watcher 不再触发」「$patch 改 N 个键只发一次事件」），从零设计，不从任何官方/上游测试改编；
- 断言 milestone 的**行为**（调用什么、得到什么），不断言实现细节；
- **行为 vs 实现细节的边界**：只断言公共 API 的输入/输出承诺（能写进文档的行为）；不断言内部存储格式、文本布局。若某个内部行为本身就是教学承诺（如「对象落盘是压缩的」），先把它写进圣经的 API 契约，再按契约断言；
- 渐进语义由第 2 步的「先红」机械保证：写出来先跑、看到红，才开始实现。

## 测试梯子：可选交付给读者的练习

`tests/` 按章 append-only、先红后绿——这份渐进结构天然是一份 TDD 作业梯子，四种实验形态皆然（library 的断言、cli-golden 的黄金输出、config-validate 的校验通过、dom-test 的 DOM 断言，都是「给出实现让测试变绿」的靶子）。

大纲声明 exercises 附录时，组装期一页成文，不占章节预算。玩法就三句：clone companion、清空 `src/`、按章序让测试从红转绿；旧章测试持续全绿，就是读者自己版本的 API 兼容哨兵。这页练习路线同时是正文「贴关键切面而非全量倾倒」的正当性来源——承重细节在测试里等着读者亲手写。

## 断点续跑与单章重生成

- 每章完成即落盘：章 md、`.course/rolling.json`（每章滚动摘要 + 是否降级 + 章文件名）。会话中断后从未完成的章继续。
- 「从第 N 章重生成」：恢复第 N-1 章的快照/滚动摘要，删去第 N..末章的测试文件，重跑 N..末章（后续章依赖前章滚动摘要，必须连锁重算）。
