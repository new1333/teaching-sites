...
目标：**不读全仓库**，产出「特性清单（8-15 个）+ 能力依赖图」。

定位先钉死：仓库是**作者侧备课资料**，只用于把特性与原理拆对。产物零仓库痕迹——正文不引源码、不列行数、不做「与真源码对照」；测试不从官方/上游测试改编；实验场 API 不刻意对齐真实库。备课阶段的克制还决定了后面 10 章还有没有上下文可用。
...
主题输入没有仓库步骤：直接把主题句拆解成同样的「特性清单 + 依赖图」（8-15 个，按学习顺序，一句话中文说明）。两条路汇流成同构数据，后续阶段完全一致——主题课程的 relevant_files 留空，无备课索引。
