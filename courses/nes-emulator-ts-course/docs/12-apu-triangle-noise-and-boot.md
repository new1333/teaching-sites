...
你现在的手里有什么：一个约 1400 行的 TypeScript NES 模拟器——CPU 能跑全部 56 条官方指令，PPU 能画背景与 64 个精灵，APU 四通道齐鸣，手柄、中断、帧时序各就各位；外加 107 个自设断言的测试梯子，每一步「先红后绿」都是原理的机械证明。更重要的是读法：回头翻第 2 章那张总图，每一块你都亲手造过。
...
- 终点：约 1400 行的自产模拟器 + 107 断言测试梯子 + 零外部输入的开机体验。
