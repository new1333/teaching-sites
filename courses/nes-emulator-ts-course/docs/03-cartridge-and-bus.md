...
这一章的第一个坑，是我在写示例代码时亲手踩的：解析器读出了正确的 PRG 长度 16KB，测试却报错——`prgRom[0]` 期望 `0xA9`，读到的却是 `0`。调试半天，发现是自造卡带的文件头多写了一个 `0`：头从 16 字节变成 17 字节，整段 PRG 悄悄右移一位，解析器毫不报错，数据全错。第二个坑更诡异：往 `$0800` 写数据，`$0000` 处的变量莫名被改——这段内存没有 bug，是硬件本来就长这样，只是我还不知道。第三个坑埋在下一章门口：CPU 开机第一件事是奔向 `$FFFC` 问「程序入口在哪」，再冲去 `$8000` 取第一条指令——如果卡带还没接进门牌段，它读回来的就是开放总线上毫无意义的脏数据，机器当场哑掉。

这三个坑指向同一件事：在 8 位机的世界里，数据放在哪、从哪读，全都由门牌号决定。这一章我们把 ROM 文件解析成卡带，再铺好 CPU 眼里那条 64KB 的门牌街——它是后面所有章节的地基。
...
```text
$0000-$1FFF │ 2KB 内部 RAM（临时草稿纸），四段镜像
...
2KB RAM 需要 11 根地址线来寻址（2 的 11 次方 = 2048）。但 CPU 报门牌用的是 16 位，那多出来的高 5 位怎么办？任天堂的省钱方案：不接。RAM 的解码电路只看低 11 位——`$0000`、`$0800`、`$1000`、`$1800` 这四段的低 11 位完全相同，高 5 位各不相同但 RAM 根本不认，于是全部命中同一块物理内存。这就是镜像（mirroring）——一间房挂了四个门牌，敲任何一个门，开的都是同一扇。

```text
CPU 报的门牌    $0805 = 0000 1 000 0000 0101
                       └高 5 位┘└── 低 11 位 ──┘
                        没接线，忽略      RAM 唯一认的部分
RAM 收到的      低 11 位 = 000 0000 0101 = $005
...
mapper（映射器）——卡带上那块换页电路的编号。大容量游戏 PRG 超过 CPU 直接寻址能力，靠它把不同页轮换到 `$8000-$FFFF` 窗口。本课程只实现 0 号（NROM，无换页、直通），这是超级马里奥兄弟等早期卡带的方案；其他编号的去向见[附录的差异清单](./ppu-apu-registers)，原理不展开。
...
有了卡带，剩下的就是那条门牌街。`Bus`（总线）的 `read`/`write` 只做一件事：按门牌段分发——谁在门牌后面，谁应答；还没接通的部件，先挂一个占位返回。先看读：

```ts
// src/bus.ts · read（全文；第 7/11/12 章的窗口先认门牌，部件到章再接）
read(addr: number): number {
  ...
  if (addr < 0x4000) {
    return this.ppu ? this.ppu.readReg(addr & 7) : 0    // PPU 窗口（第 7 章接通）
  }
  if (addr < 0x4020) {
    if (addr === 0x4016) return this.controller ? this.controller.read() : 0  // 手柄（第 12 章）
    return this.apu ? this.apu.readReg(addr) : 0        // APU 窗口（第 11 章）
  }
...
写是同一张分发表的另一半——第二个坑「写 `$0800` 改到 `$0000`」的答案就在第一行：
...
write(addr: number, val: number): void {
  ...
  val &= 0xff
  if (addr < 0x2000) {
    this.ram[addr & 0x7ff] = val         // 与 read 同一掩码：写 $0800 就是写 $0000
    return
  }
  if (addr < 0x4000) {
    this.ppu?.writeReg(addr & 7, val)    // PPU 窗口（第 7 章接通）
    return
  }
  if (addr === 0x4014 && this.ppu) {     // OAM DMA：整页灌进精灵表（第 7 章展开）
    const page = val << 8
    this.ppu.oamDma(i => this.read(page + i))
    return
  }
  if (addr === 0x4016) {
    this.controller?.write(val)          // 手柄采样脉冲（第 12 章展开）
    return
  }
  if (addr < 0x4020) {
    this.apu?.writeReg(addr, val)        // APU 窗口（第 11 章接通）
    return
  }
...
没插卡带或访问无人区时返回 0，这是开放总线（open bus）——真机上读无人认领的门牌，数据线上会残留最后一次总线读写留下的字节；本课程把它简化为恒 0。这是一个声明过的简化：真机程序也不该依赖开放总线的具体取值，恒 0 不影响任何教学结论。
...
```ts
// src/fixtures/index.ts · makeNromCartridge（全文）
...
顺手还落了 `prgWithReset`：把程序放进 16KB PRG 并在末两格写好 `$FFFC/$FFFD`——这对门牌叫复位向量，CPU 开机第一件事就是来这读入口地址。它是谁、为什么在卡带末尾，下一章 CPU 开机的瞬间你会看得清清楚楚。它的全部实现就六行（`src/fixtures/index.ts`）：铺一张 16KB 的空白 PRG、把程序字节摆进去、末两格写下入口地址的低/高字节。
...
10 个用例全绿（另一道门槛 `pnpm typecheck`——`tsc --noEmit`——同轮通过），其中最值钱的是内容级的两条：PRG 首字节原样落在 `$8000`、`$FFF0` 对应 PRG 末区——正是开头那个「多写一个 0」的坑逼出来的。
