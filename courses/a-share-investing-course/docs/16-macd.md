...
行情软件把 MACD 放在主图下方的副图：两条线加一排围绕零轴伸缩的柱。下图就是同一段行情的主图与副图——72 根 K 线，前 36 根特意排平作暖机（暖机——指标头 26 根还没成形，排平免得成形前的噪声冒出假拐角、假交叉），之后陡升、缓升、回落。读数全部出自实验场 `macd()` 的真实计算，金叉死叉标记是导出段按上述判据扫出的教学件（不进 src）。这是可交互版：鼠标十字光标停在任何一天，主图与副图同步亮出当天的 OHLC 与 DIF/DEA/柱值，也能拖动底部滑块（或滚轮）缩放区间。
