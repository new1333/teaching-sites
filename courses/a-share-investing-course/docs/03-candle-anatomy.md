...
这一章把这层压缩拆开：四个数怎么来，怎么用代码把一天的逐笔成交亲手聚成一根蜡烛，日线又怎么并成周线。从本章起，本课程有了一个伴生实验场（companion）：一个几百行的 TypeScript 最小技术分析引擎，每章演进一点。课程的每条原理，都能在里面跑给你看。先立一条阅读约定：**正文里的代码段一律默认折叠**（标着「实验场代码·点击展开」的小块），不读代码、只看图和手算，同样能走完全课程——想核对「图上的数真是算出来的」时再点开不迟。
...
为什么偏偏挑这四个数？因为一场多空拉锯战读完战报，只需要四个坐标：开局谁占先手、终局谁拿下、全天最多推进到哪里、最少退到哪里。开局与终局各来自一场集合竞价——第 2 章看过，开盘价是 9:25 全场按最大成交量投出来的，收盘价由 14:57–15:00 的收盘集合竞价定格。最高价与最低价是全天所有成交价的极值——多数时候出自连续竞价里走得最远的主动吃单（买方把卖方的队吃到最深处的那一笔打下当日最高，卖方反攻到最低的那一笔同理），但跳空高开的日子，最高价也可能直接就诞生在 9:25 那个集合竞价价上。
...
实验场的约定只有几条：`src/` 放引擎，`tests/` 放每章的断言；一切蜡烛数组按时间从旧到新排列（下标 0 最旧），这是本课程的总线约定；两道硬门槛把门——`npm run typecheck` 与 `npm test`，两道全绿，正文才动笔。
...
/** mulberry32：确定性伪随机数生成器——同一种子永远吐出同一串数，本课程合成数据的唯一随机源 */
...
3. 手机：切到周线图读本周最高价，再回日线图找这五天里最高的那个最高。又一致。一处小口径要预告：行情软件的周线按自然周聚合，遇法定假日会少并几天；实验场的周线按固定五根切一段。两种口径在没有假期的周里一致，逢春节、国庆前后的短周会对不上——那是口径差异，不是你数错了。三处对照做完，「对不上」的病就好了。
