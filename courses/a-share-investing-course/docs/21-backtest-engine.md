...
// 图三：回测明细——同一次回测的净值、基准、逐格回撤与 17 个成交点位，交给交互组件 BacktestChart。
import detail from './assets/data/21-backtest-detail.json'
</script>

你在行情软件里调出了一个「回测年化 50%」的金叉策略。回测曲线一路向右上，三年翻了三倍多；充钱实盘三个月，账户亏成狗，曲线一路向右下。你怀疑软件坏了，其实是回测这门手艺里藏着三个坑，坑坑都能把彩排成绩吹上天：信号偷看了答案、参数把历史答案背了下来、样本里只装了活下来的股票。三个坑在行内各有名字——未来函数、过拟合、幸存者偏差，本章会把它们一一亲手造出来、亲眼看它吹出多大的牛皮，再学防。
...
读数：策略做了 8 笔，总收益 −1.3%，最大回撤 41.2%，胜率 38%（3/8），盈亏比 1.85；买入持有基准 +40.3%，回撤 27.4%。两个教训当场立住。第一，均线交叉在这种震荡行情上反复挨打（第 11 章讲滞后时就预告过），既没跑赢基准、回撤还更深——**连「什么都不做」都跑不赢的策略，回测成绩单上就该打回重造**。第二，胜率 38% 配盈亏比 1.85 是第 20 章教过的画像：输多赢少但赢的大，期望值未必是负的——四件套要合起来读，单看任何一个都会误诊。

这张图由 companion 的 `src/backtest/engine.ts` 的 `backtest` 现场算出（`npm run export-docs`，种子 2102），数据文件 `21-equity.json`。

## 交互式回看：每一笔交易与每一段回撤

静态曲线只给结论，这张交互图让你亲手对账——同一行情、同一引擎、同一策略，读数与上一节一字不差。怎么看：上半区橙色实线是策略净值、青色虚线是买入持有基准（纵轴把 10 万本金记作 100）；净值旁的红上箭头是买入、绿下箭头是卖出，鼠标停在箭头上读成交价；下半区那片绿色「水下面积」是从历史峰值的逐日回撤深度；底部滑块拖动两端，可以缩放到任意时间段。

<BacktestChart :dates="detail.dates" :equity="detail.equity" :benchmark="detail.benchmark" :trades="detail.trades" :drawdown="detail.drawdown" title="守规策略 300 根行情的回测明细：净值、回撤与全部成交点" />

四步验货。第一，看曲线最右端：策略停在 98.7、基准停在 140.3——上一节的 −1.3% 与 +40.3% 就是这两条线的终点，鼠标停到途中任意一天，当日净值、基准与回撤一起报出来。第二，数一数箭头：图上共 17 枚，正好是 8 笔完整的买与卖，外加最后一枚红箭头——期末仍持有的那笔买入（tooltip 里写着「买入 @14.98（期末仍持有）」）；成绩单只数 8 笔，因为没平仓的仓位不进胜率，引擎这条规矩在图上也看得见。第三，下半区绿色面积扎得最深的一格是 −41.2%，与上一节的最大回撤一字不差——回撤序列从 0 出发，净值每跌破此前的高点就向下沉一格，这片面积就是「从山顶摔下来有多深」。第四，拖动底部滑块放大任意一段：哪天进的、哪天出的、卖完行情怎么走，都能定格到具体日期。本章的两张静态资金曲线图（上一节一张、下一节一张）负责把道理讲清，这张交互图管验货——回测报告里的每个数字，都该能在图上指出它的位置。
...
读数：偷看写法整段只做一笔（2026-09-17 开盘买、2027-02-08 开盘卖），总收益 +89.9%，最大回撤 6.8%——回撤比守规版浅六倍。注意这台引擎并没有放水：成交照样 T+1 次日开盘、照样扣三件套费用，偷看发生在信号层面——「最低点」「最高点」是拿全序列事后挑出来的，实盘那天你根本不知道哪里是最低。开章那句「回测年化 50%」，最常见的就是这台造币机印出来的。ZIGZAG 这类会「重绘」的指标（新 K 线出来后回头改写旧信号）、软件默认的「当天收盘出信号、按当天收盘价成交」口径（信号成立的那一刻价格已经过去了），都是它的变体。
...
终端在旧摘要之后追加第 21 章一段：守规的 MA5/20 交叉策略 8 笔、总收益 −1.3%、最大回撤 41.2%、胜率 38%、盈亏比 1.85；买入持有基准 40.3%、回撤 27.4%；偷看写法整段一笔、总收益 89.9%、最大回撤 6.8%；明细图把 8 笔交易摊成 17 个点位、逐格回撤最深 −41.2%。`docs/assets/data/` 下多出三个 `21-*.json`。再跑一遍，一个字节都不变。

不进实验仓也有载体——一笔来回的真实成本，纸笔就能复算：

<details>
<summary>🔧 实验场代码 · 点击展开</summary>
...
去向一句话：引擎已经能给出含费用、守规则的成绩单，第 22 章把本课程积木——形态、指标、期望值、仓位、回测——组装成一套完整的交易系统，并配一份防骗清单。
