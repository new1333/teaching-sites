...
读完本课程，最强的验证方式不是再做一遍笔记，而是把引擎删掉重写一遍。实验场的测试是按章解锁的作业梯子：每章的测试文件就是那章的目标规格，你从零实现 `src/`，让测试逐章从红转绿。
...
3. **对照终态**：本课程测试都绿后（应为 430 项），你的实现与课程终态在行为上等价；写法不同完全正常——测试断言的是行为，不是实现细节。

不打算写代码？这条路可以整个跳过：每章正文末尾的「验证」小节（纸笔跟算 + 手机实操）就是不进实验场的读者的完整练习路线，配合两张速查表同样能走完本课程。

## 各章测试 ↔ 教的什么

| 测试文件 | 章 | 你要重建的能力 |
|---|---|---|
| `auction-matching.test.ts` | [第 2 章](./02-order-journey) | 集合竞价最大成交量定价与成交分配 |
...
第 1、10、19、22 章是原理章，没有测试文件——它们的「作业」在正文末尾的自查问里。
