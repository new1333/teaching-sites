...
// 图二：同一段行情的 RSI 副图。
import rsiPanel from './assets/data/17-rsi-panel.json'
// 图三：同一段行情的 KDJ 副图。
import kdjPanel from './assets/data/17-kdj-panel.json'
// 图四：钝化窗口的行情（切片视图）。
...
跟着算三步。第 4 根：前三根的涨幅列是 0、0、3，平均涨幅 1，平均跌幅 0，RSI = 100×1÷(1+0) = 100。第 5 根：旧平均扛 2/3、新值占 1/3，平均涨幅 =（1×2+1）÷3 = 1，平均跌幅仍为 0，还是 100。第 7 根：跌幅列进来一个 2，平均跌幅 =（0×2+2）÷3 = 0.667，平均跌幅一路追到 0.444，RSI = 100×0.444÷1.111 = 40.0。注意第 4～6 根：三根读数一字不差全是 100——跌幅那列为零，分母只剩下分子，比值钉死。这不是巧合，是本章后半的主角。
...
同一段拉升行情的 RSI 副图如下。70/30 两条刻度线的位置，正是后文「钝化」发生的地方。

<LineChart :series="rsiPanel.series" :labels="rsiPanel.labels" title="同一段行情的 RSI 副图（70/30 刻度）" />
...
窗口内最高与最低重合（一字横盘）时分母为零，RSV 记 50——与走平的 RSI 同一个约定。下面是同一段行情的 KDJ 副图，K/D/J 三线与 80/20 刻度线全部由 `kdj()` 算出。

<LineChart :series="kdjPanel.series" :labels="kdjPanel.labels" title="同一段行情的 KDJ 副图（K/D/J）" />
