...
import matrix from './assets/data/12-matrix-candles.json'
// 量价矩阵四格各一段八根迷你样本：主图看价格台阶、副图看量台阶，同向或反向四格各一张。
</script>

四月中旬，你在另一只票上又清了一次仓。理由只有一句：价涨量缩，要反转。那天收盘 13.10 元，比前一日又涨了一点，成交量（当天实际成交的股数）却比前五日的平均小三成。群里都说量跟不上、主力在拉高出货，你越想越怕，尾盘（临近收盘的时段）全卖了。之后三周它没回头：13.1 → 14.2 → 15.6，最后收在 18 元上方——主升浪（一波行情里最陡、最连贯的中段拉升）你只坐了前半程。复盘时你想给自己找个说法，才发现问题根本不在卖错：你依据的是一句口诀，不是判据。量「跟不上」——跟谁比？缩到几成才算缩？价格的「新高」又是哪个窗口里的高？口诀一个都没答。
...
方向由谁主动决定，规模由量记账。买方急着成交，就往上吃卖单，价格涨；卖方急着走，就往下砸买单，价格跌。**价格是最后一笔成交的读数，量是全部成交的合计**（日线语境下，「最后一笔」通常指 14:57–15:00 收盘集合竞价定格的那笔——第 2 章的时段表）——这个不对称就是「量在价先」的全部机制，两条推论：
...
| 价跌量增 | 恐慌或派发（大资金趁有人肯接时慢慢卖出）：急着卖的撞上肯接的 | 放量破位、放量下跌 | 不接飞刀（不在价格加速下坠时逆势抄底）；等量能衰竭再谈企稳 |
| 价跌量缩 | 卖压枯竭：想卖的卖得差不多了 | 阴跌尾段、地量区 | 地量不是买点，是变盘临近的观察窗；右侧确认再进场（等价格真正转头、走出回升结构再动手，不抢在转折前） |

表格给的是判据，眼睛还没见过这四格长什么样。下面四张迷你图按表格顺序排布，每张八根 K 线：主图看价格往哪边走，量副图看量的台阶往哪边走——两眼对齐，格子的长相就记住了。

<KLineChart :candles="matrix.scenes[0].candles" :height="200" :show-volume="true" :title="matrix.scenes[0].label" />

第一格，价涨量增：K 线从 10.07 元抬到 11.21 元，量副图从 200 万股台阶式放大到 700 万股——推进有燃料，每一级新高都是放量买出来的。

<KLineChart :candles="matrix.scenes[2].candles" :height="200" :show-volume="true" :title="matrix.scenes[2].label" />

第二格，价涨量缩：价格从 10.09 元爬到 10.87 元，量副图却从 700 万股一级级枯到 200 万股——无人反对的推进。这格的读法全看位置：启动初期的惜售无害，高位连续缩量新高才升级为警示。

<KLineChart :candles="matrix.scenes[1].candles" :height="200" :show-volume="true" :title="matrix.scenes[1].label" />

第三格，价跌量增：K 线从 9.91 元逐根走低到 9.13 元，量副图反而从 200 万股放大到 700 万股——急着卖的撞上肯接的，恐慌或派发，分歧在放大。

<KLineChart :candles="matrix.scenes[3].candles" :height="200" :show-volume="true" :title="matrix.scenes[3].label" />

第四格，价跌量缩：价格阴跌到 9.03 元，量副图一路枯到 200 万股——想卖的卖得差不多了。地量不是买点，是变盘临近的观察窗，右侧确认再进场。
...
守门同样内置在脚本里。标签全景图必须四类标签各就各位、且零背离——横盘里的量缩新高是噪声级信号，不配当主角，脚本已把那些 K 线的收盘价钉进前五根区间的正中。背离图的前 50 根量价同步段必须零背离。两段缩量各须扫出至少一枚背离。任一守门失手，整段导出失败。简化之处照实声明：1.5 与 0.7 的倍数线、5 根参照窗、20 根极值窗都是课程操作化选择；背离用「当根量对近五日均量」的口径，教科书常见的「峰对峰量能对比」需要枢轴识别器，本课程不提前实现；量价矩阵取「量分增缩、价分涨跌」的四格版，经典的量价八法里「量平」「价平」两类低信息组合不单列；行情与量能阶梯均为固定种子合成数据。全部登记在附录差异清单。
...
终端在旧摘要之后追加第 12 章一段：标签全景记标签 16 枚（放量 6、缩量 8、天量 1、地量 1）、零背离；背离图两段缩量各扫出顶背离 3 处、第 16 根 1.8 倍放量无背离；换手率曲线天量日 3%。`docs/assets/data/` 下多出四个 `12-*.json`。再跑一遍，一个字节都不变。
