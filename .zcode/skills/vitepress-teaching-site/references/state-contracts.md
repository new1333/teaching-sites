...
**覆盖与数量**：必须覆盖相邻知识、熟练度、使用场景、期望终点四个维度，跳过任一维度要在 `default_reason` 显式记录。每维度 2-3 问，总数以 12 为上限；取值向多问一侧倾斜——拿不准读者水平时优先追加判别式问题，而不是落默认假设。某维度确实凑不出第二问时，在 `decisions` 记录原因后可只问 1 问；确需超出 12 问时逐问在 `decisions` 写明它改变哪项课程决策。
